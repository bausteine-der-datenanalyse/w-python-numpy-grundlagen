...
Reihen in Spalten und Spalten in Reihen umgewandelt.
...
Wollen wir nun aus dieser Matrix einen Vektor erstellen, so können wir die Methode `np.flatten()` benutzen:
...
Position im Array einzufügen. Das passende Werkzeug ist hierfür die Funktion 
`np.insert(Array, Position, Einschub)`. Im folgenden Beispiel wird an der 
Indexposition 3 die Zahl 7 eingesetzt.
...
Möchte man nun eine Liste erstellen, die jede Nummer nur einmal enthält, kann die Funktion `np.unique()` verwendet werden. 
