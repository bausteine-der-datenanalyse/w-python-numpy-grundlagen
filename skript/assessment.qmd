...
Themen führt. Alternativ finden Sie darunter normale Fragen, wie Sie bisher im 
...
3. Ein zweidimensionales Array der Form 3×3, das nur aus Einsen besteht.
4. Ein eindimensionales Array, das die Zahlen von 10 bis 50 (einschließlich) in 
Schritten von 5 enthält.

## Aufgabe 3 {.unnumbered}

Was ist der Unterschied zwischen den Funktionen `np.ndim()`, `np.shape()` und
`np.size()`?
...
2. Bestimmen Sie Minimum und Maximum des Arrays.
...
5. Transponieren Sie die 4×5-Matrix aus Aufgabe 1.
...
Sie möchten aus einem Bild die Bilddaten einer Farbkomponente isolieren. Was
...
Typischerweise sind Bilddaten große Matrizen, wobei die Farben in drei unterschiedlichen Matrizen gespeichert werden. Dabei ist die Farbreihenfolge oft "Rot", "Grün" und "Blau". Dementsprechend müssen wir, wenn die Daten in der Matrix `data` gespeichert sind, mit Slicing eine Dimension auswählen: `data[:,:,0]`, wobei die Zahl 0-2 für die jeweilige Farbe steht.
:::