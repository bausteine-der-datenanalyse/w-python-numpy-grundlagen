...
Wenn man sich nicht mehr sicher ist, welche Struktur oder Form ein Array hat
...
`np.shape()` gibt die Längen der Dimensionen in Form eines Tupels zurück. 
```{python}
np.shape(matrix)
```
...
Die Funktion `np.ndim()` gibt im Gegensatz zu `np.shape()` nur die Anzahl der
Dimensionen zurück. 
...
NumPy-Arrays können verschiedene Datentypen haben. Im Folgenden haben wir 
drei Arrays mit jeweils unterschiedlichem Datentyp.
...
| Positive ganze Zahl   | `uint`     | [1, 2, 3, 4, 5]            |
| Kommazahl             | `float`    | [1.3, 7.4, 3.5, 5.5]      |
| komplexe Zahl         | `complex`  | [-1 + 9j, 2-77j, 72 + 11j] |
...
Dimension. Von welchem Datentyp ist der Inhalt dieser Matrix?
